...
const ReviewsSection = () => (
    <div id="resenas" style={{ maxWidth: '600px', margin: '60px auto', padding: '30px', backgroundColor: 'rgba(0, 0, 0, 0.7)', borderRadius: '15px', color: 'white', border: '1px solid rgba(255,255,255,0.2)' }}>
        ...
        {/* Usando tu ID real: mpwvvveb */}
        <form action="https://formspree.io/f/mpwvvveb" method="POST" style={{ textAlign: 'left' as 'left' }}>
            <input name="Nombre" placeholder="Tu Nombre" required style={{ width: '100%', padding: '12px', marginBottom: '10px', borderRadius: '8px', border: 'none', backgroundColor: '#fff', color: '#000' }} />
            <select name="Estrellas" style={{ width: '100%', padding: '12px', marginBottom: '10px', borderRadius: '8px', border: 'none', backgroundColor: '#fff', color: '#000' }}>
                <option value="5">⭐⭐⭐⭐⭐</option>
                <option value="4">⭐⭐⭐⭐</option>
                <option value="3">⭐⭐⭐</option>
            </select>
            <textarea name="Comentario" placeholder="¿Qué te pareció el entrenamiento?" required style={{ width: '100%', padding: '12px', borderRadius: '8px', border: 'none', minHeight: '60px', backgroundColor: '#fff', color: '#000' }} />
...
const RequestForm = () => {
    const inputStyle = { width: '100%', padding: '12px', margin: '8px 0', borderRadius: '8px', border: '1px solid #ccc', backgroundColor: '#ffffff', color: '#000000', fontSize: '16px' };
    return (
...
export default function Home() {
    return (
        <div style={{ textAlign: 'center', minHeight: '100vh', backgroundImage: 'url("/IMG-20251215-WA0059.jpg")', backgroundSize: 'cover', backgroundAttachment: 'fixed', backgroundPosition: 'center', scrollBehavior: 'smooth' }}>
            <SideNav />
            <audio controls autoPlay loop style={{ position: 'fixed', bottom: '15px', left: '15px', zIndex: 1000, width: '150px', height: '30px', opacity: 0.6 }}><source src="https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3" type="audio/mpeg" /></audio>
            <div id="presentacion" style={{ paddingTop: '100px', marginBottom: '40px' }}>
                ...
                <a href="#contacto" style={{ display: 'inline-block', marginTop: '30px', padding: '15px 30px', backgroundColor: '#FFD700', color: 'black', textDecoration: 'none', borderRadius: '50px', fontWeight: 'bold', fontSize: '1.1em', boxShadow: '0 4px 15px rgba(255,215,0,0.4)' }}>🔥 QUIERO MI RUTINA PERSONALIZADA</a>
            </div>
            <VideoGallery />
            <ReviewsSection />
            <FitnessTipsSection />
